...
fn main() -> Result<()> {
    ...
    if version == "all" {
        if let Err(_) = verify_all_versions(test_output, quiet) {
            process::exit(1);
        }
    } else if let Ok(v) = version.parse::<Version>() {
        if let Err(_) = verify_version(v, test_output, quiet) {
            process::exit(1);
        }
    } else {
...
fn verify_all_versions(test_output: Option<&String>, quiet: bool) -> Result<()> {
    ...
    for version in VERSIONS {
        ...
        if let Err(_) = verify_version(version, test_output, quiet) {
            any_failed = true;
        }
    }
    if any_failed {
        Err(anyhow::anyhow!("verification failed for one or more versions"))
    } else {
        Ok(())
    }
}
...
fn verify_version(version: Version, test_output: Option<&String>, quiet: bool) -> Result<()> {
    ...
    if failures == 0 { Ok(()) } else { Err(anyhow::anyhow!("verification failed ({} check(s) failed)", failures)) }
}
...
fn verify_correct_methods(version: Version, methods: Vec<String>, msg: &str) -> Result<()> {
    ...
    if verify::correct_methods(&got, &want, msg) { Ok(()) } else { Err(anyhow::anyhow!("incorrect {}", msg)) }
}
...
fn verify_status(version: Version, test_output: Option<&String>) -> Result<()> {
        ...
        match method.status {
            Status::Done => {
                ...
                if let Some(test_output) = test_output {
                    if let Err(_) = check_integration_test_crate::test_exists(version, &method.name, test_output) {
    ...
    if failures == 0 { Ok(()) } else { Err(anyhow::anyhow!("status verification failed ({} issue(s))", failures)) }
}
