...
fn blockchain__scan_blocks_modelled() {
    let node = match () {
        ...
        () => Node::with_wallet(Wallet::None, &["-blockfilterindex=1"])
    };
    ...
    let json: ScanBlocksStart = node.client.scan_blocks_start(&[&scan_desc], None, None, None, None).expect("scanblocks start");
